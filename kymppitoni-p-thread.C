...
void *thread_function (void *thread_var)
{
	long tid;
	tid = *((int*)thread_var) + 1;

	long long local_inplay_throws[7];
	long long local_inplay_points[7];
	long long local_inplay_duds[7];

	for(int i=1; i<7; i++){
	    local_inplay_duds[i] = 0;
	    local_inplay_throws[i] = 0;
	    local_inplay_points[i] = 0;
	}
	
	for (int c = ((tid-1) * 50) + mincutoff; c < maxcutoff; c += (50 * MAX_THREADS))
	{
...
int main( int argc, char** argv) {
  ...
  games = atoi(argv[1]);
  mincutoff = atoi(argv[2]);
  maxcutoff = atoi(argv[3]);
...
//  long long total_inplay_duds[7];
  ...
  max_turns = 0;
  min_turns = games*10000; // larger than should be possible
  max_cutoff;
  min_cutoff;
  ...
  for (long tid =1 ; tid < MAX_THREADS; tid++)
  {
	int *arg = (int *)malloc(sizeof(arg));
	*arg = tid;
	pthread_create (&pid[tid], NULL, thread_function, (void*)arg);
  }
